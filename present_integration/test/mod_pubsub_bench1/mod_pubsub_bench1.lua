...
function simple_broadcast(kind, node, jids, item, actor, node_obj)
	...
	if kind == "items" then
		if node == "reverse" then 
			module:log("info","%s",tostring(item));
			local payload1 = item:get_child("content","http://jabber.org/protocol/pubsub"):get_text();
			module:log("info","%s",payload1);
			payload1 = string.reverse(payload1);
			message = st.message({ from = module.host, type = msg_type, id = id })
				:tag("event", { xmlns = xmlns_pubsub_event })
					:tag(kind, { node = node })
					:tag("reverse"):text(payload1)
